...
import { getPendingOrders } from "@/client/api/admin";
...
const getEnabled = () => {
  try { return typeof window !== "undefined" && localStorage.getItem(STORAGE_KEY) === "on"; }
  catch { return false; }
};
...
function setEnabledPersist(on: boolean) {
  try {
    ...
    toast.error("O navegador não permitiu salvar a preferência de som.");
  }
}
...
export function OrderChime() {
  ...
  React.useEffect(() => {
    ...
    const tick = async () => {
      ...
      inFlight = true;
      try {
        const res = await getPendingOrders(unit, { signal: controller.signal });
  ...
  return (
    <Button
      ...
      className={
        ringing
          ? "animate-pulse text-brand-600 motion-reduce:animate-none dark:text-brand-400"
          : enabled
