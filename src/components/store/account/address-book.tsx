...
function AddressForm({
  ...
  const cepRequest = React.useRef<AbortController | null>(null);
  ...
  React.useEffect(() => () => cepRequest.current?.abort(), []);
  ...
  async function handleCep(e: React.FocusEvent<HTMLInputElement>) {
    ...
    const found = await lookupCep(zip, { signal: controller.signal }).finally(() => {
      if (!controller.signal.aborted) setCepLoading(false);
    });
    const currentZip = (formRef.current?.elements.namedItem("zip") as HTMLInputElement | null)?.value;
    if (controller.signal.aborted || currentZip !== zip || !found || !formRef.current || submissionLock.current) return;
  ...
  return (
    <form
      ...
      onSubmit={(event) => {
        ...
        else { submissionLock.current = true; cepRequest.current?.abort(); }
      }}
...
export function AddressBook({ addresses }: { addresses: Address[] }) {
  ...
  const onDelete = (a: Address) => confirmation.confirm({
    title: `Excluir endereço ${a.label}?`,
    confirmMessage: "O endereço será removido da sua conta. Endereços já registrados em pedidos serão preservados.",
                ...
                <div className="flex shrink-0 items-center gap-1">
                  {!a.isDefault && (
                    <button
                      ...
                      disabled={pending || confirmation.pending}
